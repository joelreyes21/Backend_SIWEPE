...
There is also a "platform admin" concept: a `users` row with `empresa_id = NULL` (created by `crear-usuario.js` with no empresa argument, or by `asegurarBase()` on first boot **only if** both `BOOTSTRAP_ADMIN_EMAIL` and `BOOTSTRAP_ADMIN_PASSWORD` — 12+ chars — are set; there is no hardcoded default credential, "Nunca crea credenciales conocidas" per its own comment). This account can authenticate but **cannot** use `/api/state` (`empresaId` is required, returns 403 without one) — it exists only so the install always has *some* admin credential, not as a cross-tenant superuser role.
